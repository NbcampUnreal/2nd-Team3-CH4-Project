...
void UGameResultWidget::PopulatePlayerEntries(const TArray<FPlayerResultData>& ResultList)
{
    ...
    TArray<FPlayerResultData> SortedList = ResultList;
    SortPlayerResultsByKill(SortedList);

    for (const FPlayerResultData& Entry : SortedList)
    {
        ...
        if (NewSlot)
        {
            NewSlot->SetPadding(FMargin(0.f, 8.f));
        }
    }
}
