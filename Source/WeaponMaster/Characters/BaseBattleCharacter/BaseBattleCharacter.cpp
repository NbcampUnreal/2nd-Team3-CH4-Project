...
void ABaseBattleCharacter::BeginPlay()
{
	...
	if (GetNetMode() != ENetMode::NM_DedicatedServer)
	{
		...
		{
			UE_LOG(LogTemp, Error, TEXT("ABaseBattleCharacter::Beginplay : PlayerState Cast Failed"));
		}
	}
}
			...
			FTimerDelegate::CreateLambda([this]()
			{
				...
				if (auto GM = Cast<IBattleGMInterface>(UGameplayStatics::GetGameMode(this)))
				{
					if (auto CastedGameInstance = Cast<UWeaponMasterGameInstance>(GetGameInstance()))
					{
						...
						{
							GM->HandlePlayerDeath(WMController, GetPlayerState()->GetPlayerName());
						}
					}
					else
					{
...
void ABaseBattleCharacter::OnAttacked(const FAttackData& AttackData)
{
	if (HasAuthority())
	{
		...
		{
			if (auto WMGS = Cast<AWeaponMasterGameState>(World->GetGameState()))
			{
				if (!WMGS->bIsPVP && IsValid(AttackData.Attacker) && !AttackData.Attacker->IsPendingKillPending())
				{
